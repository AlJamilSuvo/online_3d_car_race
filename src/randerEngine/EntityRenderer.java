...
public class EntityRenderer {
	...
	public void render(Map<TextureModel,List<Entity>> entities){
		for (TextureModel model:entities.keySet()){
			shader.loadUsingNormal(model.isUsingNormal());
	...
	public void render(Player player){
		...
		for(TextureModel textureModel:textureModels){
			shader.loadUsingNormal(textureModel.isUsingNormal());
			RawModel rawModel=textureModel.getRawModel();
			GL30.glBindVertexArray(rawModel.getVaoID());
			GL20.glEnableVertexAttribArray(0);
			GL20.glEnableVertexAttribArray(1);
			GL20.glEnableVertexAttribArray(2);
			shader.loadFackLight(textureModel.getModelTexture().isUseFakeLighting());
