...
class MainGame {
	...
	public MainGame(Vector3f playerPos, Vector3f playerRot, List<Vector3f> otherPos, List<Vector3f> otherRot,List<Vector2f> otherV) {
		...
		//collide.put(treeModel,OBJFileLoader.loadOBJ("tree_collide"));
		...
		try{
			...
			while((line=reader.readLine())!=null){
				...
				System.out.println(str[0]+" "+collideData);
				if (collideData==null){
					System.out.println(str[0]+" "+"Not Found");
					//collideData=OBJFileLoader.loadOBJ("wall collide");
				}
	...
	private void collision() {
		...
		for (Entity entity : entities) {
			...
			if (Maths.getDistance(player, entity) < 100) {

				if (Collision.testInterSection(player.getEntity(), entity)) {
				//if(Collision.testInterSection(player.getEntity(),player.getVelocity(),entity,new Vector3f(0,0,0),DisplayManager.getFrameTimeSeconds()*2)){
					player.velocity.x*=-1.02f;
					player.velocity.z*=-1.02f;
					if(player.velocity.length()<5){
						player.velocity.x+=5*Math.abs(player.velocity.x)/player.velocity.x;
						player.velocity.z+=5*Math.abs(player.velocity.z)/player.velocity.z;
					}
		...
		for (EntityPack entity : others) {
			...
			if (Maths.getDistance(player, entity) < 50) {
				if (Collision.testInterSection(player.getEntity(), entity)) {
			//	if(Collision.testInterSection(player.getEntity(),player.getVelocity(),entity,entity.getVelocity(),.1f)){
